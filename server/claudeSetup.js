...
function writeMcpConfig(dir, base, sessionId) {
  const file = path.join(dir, `mcp-${sessionId}.json`);
  fs.writeFileSync(file, JSON.stringify({
    mcpServers: {
      cctower: {
        command: process.execPath,
        args: [path.join(__dirname, 'mcpReport.js')],
        env: { CCW_SESSION_ID: sessionId, CCW_BASE_URL: base },
      },
    },
  }, null, 2));
  return file;
...
function protocolPrompt(base, sessionId) {
  ...
  return [
    ...
    'Report with the mcp__cctower__report_status tool (pre-approved, preferred). Only if that tool is unavailable, fall back to Bash:  curl -s -m 3 -X POST \'' + url + '\' -H \'Content-Type: application/json\' -d \'<JSON matching the tool schema>\'',
