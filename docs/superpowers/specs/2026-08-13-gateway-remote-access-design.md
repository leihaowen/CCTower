...
| origin 隔离 | **一期有意不做**(终审 I-6,详情见下):所有被代理机器与网关 UI 共用同一 origin,风险与解决条件见下方说明 |

一期有意不做:TOTP(三期随账号体系)、网关高可用(单点,agent 自愈重连)、审计日志。

- **origin 隔离(终审 I-6)**:`/s/<id>/` 是路径式代理,不是子域名隔离——网关自身的 UI 与
  N 台被代理的 CCTower 全部塌缩到同一个 origin(同一个 scheme+host+port)下。风险:
  任一台被代理机器上的 XSS(`public/app.js` 有大量 `innerHTML` 插入点),或一台机器本身被
  攻陷,都可以借用户当前登录会话的同源 cookie(HttpOnly 挡不住同源请求自动携带 cookie),
  用 `fetch` 去调网关自己的 `/api/overview`,或跨过去调其它每一台机器的 `/s/<other-id>/api/...`——
  而 CCTower 的 API 本身就等价于在那台机器上执行任意命令。也就是说,一台机器失守,
  同一网关下的其它机器在攻击面上并不完全独立。
  一期接受这条取舍的原因:真正的隔离需要每台机器一个独立子域名(`server-id.cc.example.com`),
  这意味着通配符证书 + 按机器分配子域名 + Cookie 作用域收窄到单个子域名,属于下一阶段
  才做的基础设施投入,一期的成本/收益不划算(目标用户是个人/小团队自己管理的少数几台机器,
  不是互不信任的多租户)。**必须解决的条件**:一旦网关开始面向互不信任的多个使用者
  (即多账号/多租户,见"隔离预留"一行),或者被代理的机器数量大到"单台失守连累全部"
  的后果不可接受,就必须在那之前把 `/s/<id>/` 换成子域名隔离,不能再靠"用户自己信任
  自己管理的每一台机器"这个假设。
  已经做的收敛(见 6.3 与 `gateway/src/proxy.js`):被代理机器的响应头如果试图设置一个
  与网关会话 cookie(`ccgw_session`)同名的 `Set-Cookie`,会被网关过滤掉——防止一台被攻陷
  的机器伪造网关会话、持续把用户踢下线或劫持登录态;但这只堵住了"改写会话"这一条路,
  同源带来的跨机器 API 调用能力本身在一期无法低成本消除。
